...
def _shutdown(sig, frame):
    print("\n[launch_producers] Shutting down all producers...")
    for p in procs:
        p.terminate()
    for p in procs:
        p.wait()
    print("[launch_producers] All stopped.")
    sys.exit(0)
...
print("[launch_producers] Starting ShopStream data generators...")
for script in PRODUCERS:
    p = subprocess.Popen([sys.executable, str(BASE / script)])
    procs.append(p)
    print(f"  ▶  {script}  (pid {p.pid})")

print(f"\n[launch_producers] {len(procs)} producers running. Press Ctrl+C to stop all.\n")
for p in procs:
    p.wait()
